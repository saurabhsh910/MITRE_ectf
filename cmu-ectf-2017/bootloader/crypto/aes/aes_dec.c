...
void aes_invshiftrow(void* data, uint8_t shift) BOOTLOADERLIBS;
...
void aes_invshiftcol(void* data, uint8_t shift) BOOTLOADERLIBS;
...
static void aes_dec_round(aes_cipher_state_t* state, const aes_roundkey_t* k) BOOTLOADERLIBS;
...
static void aes_dec_firstround(aes_cipher_state_t* state, const aes_roundkey_t* k) BOOTLOADERLIBS;
...
void aes_decrypt_core(aes_cipher_state_t* state, const aes_genctx_t* ks, uint8_t rounds){
	...
	uint8_t loc_i;
	aes_cipher_state_t loc_state;

	// SCP
	memcpy(&loc_state,state,sizeof(aes_cipher_state_t));
	for (loc_i = 0;loc_i < sizeof(aes_cipher_state_t);loc_i++)
	{
		loc_state.s[loc_i] = ~(loc_state.s[loc_i]);
	}

	aes_dec_firstround(state, &(ks->key[i=rounds]));
	aes_dec_firstround(&loc_state, &(ks->key[i=rounds]));

	for(;rounds>1;--rounds){
		--i;
		aes_dec_round(&loc_state, &(ks->key[i]));
