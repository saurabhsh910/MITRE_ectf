...
void aes_shiftcol(void* data, uint8_t shift) BOOTLOADERLIBS;
...
static void aes_enc_round(aes_cipher_state_t* state, const aes_roundkey_t* k) BOOTLOADERLIBS;
...
static void aes_enc_lastround(aes_cipher_state_t* state,const aes_roundkey_t* k) BOOTLOADERLIBS;
...
// Already declared as BOOTLOADERLIBS in header
void aes_encrypt_core(aes_cipher_state_t* state, const aes_genctx_t* ks, uint8_t rounds){
	...
	i=1;
	for(;rounds>1;--rounds){
		aes_enc_round(&loc_state, &(ks->key[i]));
		aes_enc_round(state, &(ks->key[i]));
		++i;
	}

	aes_enc_lastround(state, &(ks->key[i]));
	aes_enc_lastround(&loc_state, &(ks->key[i]));
}
